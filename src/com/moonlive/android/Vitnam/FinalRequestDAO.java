...
import com.moonlive.android.Vitnam.model.LiveUrl;
...
import com.moonlive.android.Vitnam.view.HorizontalMessageView;
import com.moonlive.android.Vitnam.view.MyMarquee;
...
//	private boolean readAuthCache = false;
	...
	//��ȡ���
	public void getProgramAd(final LiveData liveData,int requestTimes){
        ...
        if(NetworkUtil.isConnectingToInternet(MyApplication.getApplication())){
        	...
        	finalHttp.get(url,new AjaxCallBack<Object>() {
        		public void onSuccess(Object t) {
        			...
        			if(!gsonStr.equals("[]"))
        				ProgramAdCache.save(gsonStr, liveData.getId());
        		};
        		
        		public void onFailure(Throwable t, int errorNo, String strMsg) {
        			int pos = adPos + 1;
        			if(pos < list.size())
        				getProgramAd(liveData,pos);
        		};
        	});
        }
	} 
...
//        			sendIntentMsg("",MsgService.FIANL_DAO_APP_MSG);
	...
	public void checkAuth(MainActivity a, Handler h){
		mActivity = a;
		mHandlerCheckAuth = h;
...
//				bundle.putString(Configs.INTENT_PARAM, vodUrl.getUrl());//�Ϸ�ʽ����Ȩ�ɹ�,��̨���ػ�ȡ��Ŀ�б��URL
	...
	public void initPopWindow(final String appUrl,
			UpdateData updata, MainActivity a, Handler h) {
		try{
			mActivity = a;
			mHandler = h;
			mUpdateUrl = appUrl;
		...
		new Thread() {
			public void run() {
				try {
					...
					handler.sendEmptyMessage(MainActivity.START_INSTALL);
					return;
	...
	public void showMsg(String paramString,MainActivity a, Handler h) {
		mHandler = h;
		mActivity =a;
		if(null == mMarquee)
    ...
    @SuppressWarnings("deprecation")
	public void showExitWindow(MainActivity a, Handler h){  
    	mActivity = a;
    	mHandler = h;
